...
class PusherFacade(threading.Thread):
    ...
    def connect_handler(data):
        channel = self.pusher.subscribe(CHANNEL)
        channel.bind('play', self.play_callback)
...
class JukeboxUI(cmd.Cmd, threading.Thread):
    ...
    def do_startListener(self, line):
        print "Starting websocket client (Pusher)..."
        self.pusher = PusherFacade()
        self.pusher.start()
        
    def do_stopListener(self, line):
